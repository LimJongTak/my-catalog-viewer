...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const scrollWrapper = document.getElementById('horizontal-scroll-wrapper') as HTMLDivElement;
    ...
    scrollWrapper.addEventListener('mousemove', (e) => {
        if (!isDown) return; // 클릭 상태가 아니면 무시
    ...
    langButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            if (window.innerWidth <= 1200) langSelect.classList.remove('dropdown-open');
        });
    });
